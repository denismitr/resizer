...
var rxCorrectKey = regexp.MustCompile(`^[a-zA-Z0-9-_]+/[a-zA-Z0-9-_]{2,100}\.(png|jpg|webp)+`)
...
func (rs *RemoteStorage) Put(ctx context.Context, namespace, filename string, source io.Reader) (*storage.Item, error) {
	if !isValidKey(filename) {
		return nil, errors.Wrapf(storage.ErrInvalidFilename, "invalid s3 key %s", filename)
	}

	if namespace == "" {
		return nil, errors.Wrapf(storage.ErrInvalidNamespace, "invalid s3 bucket [%s]", namespace)
	}
...
func isValidKey(key string) bool {
	return rxCorrectKey.MatchString(key)
}